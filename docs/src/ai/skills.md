...
Type `/` in the message editor to see all available skills as slash commands. Selecting one injects its instructions into the conversation as context, and the agent responds using them.

For example, typing `/frontend-design` loads that skill and the agent follows its design workflow for the rest of the conversation.

### Model invocation {#model-invocation}

By default, the agent can also load skills autonomously. It sees a catalog of every installed skill (name and description) in its system prompt, and calls the `skill` tool when a task matches a skill's description.
